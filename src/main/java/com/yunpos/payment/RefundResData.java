...
public class RefundResData {
	...
	// 支付返回类型数据转换
	public RefundResData(PayChannel channel, Map<String, String> resMap, Map<String, String> map,
		...
		if (channel.equals(PayChannel.ALIPAY)) {// 支付宝
			...
			this.dynamic_type = "2";
			this.trans_type = "";
			this.trans_amount = map.get("refund_amount");
			this.total_fee = map.get("refund_amount");
			this.merchant_name = map.get("merchant_name");
