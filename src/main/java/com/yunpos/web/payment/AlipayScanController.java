 ...
 */
@Controller
@RequestMapping("/pay/alipay/scan")
public class AlipayScanController {
	Logger log = LoggerFactory.getLogger(AlipayScanController.class);
	...
	public Object alipayCreate(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			AlipayPrecreateReqData alipayPrecreateReqData = new AlipayPrecreateReqData(orderNo, AlipayConfig.partner,
					"扫码预支付", total_fee);

			payMsg = alipayService.preCreate(alipayPrecreateReqData);
		} catch (Exception e) {
			log.error("支付出现异常：", e);
	...
	// @ResponseBody
	// public Object query(HttpServletRequest request, HttpServletResponse
	// response) {
	// String pay_channel = request.getParameter("pay_channel");
	...
	// || Strings.isNullOrEmpty(trace_num) ||
	// Strings.isNullOrEmpty("terminal_unique_no")) {
	// return new Message(ResultCode.FAIL.name(),
	// ErrorCode.PARAM_IS_NULL.name(), "传递参数为空！", null);
	// }
	// Message payMsg = null;
	// try {
	// // 商户信息从数据库配置中获取
	// // -------------------------------
	// AlipayQueryReqData alipayQueryReqData = new AlipayQueryReqData(trace_num,
	...
	// log.error("支付出现异常：", e);
	// return new Message(ResultCode.FAIL.name(),
	// ErrorCode.SYSTEM_EXCEPTION.name(), "支付出现异常！", null);
	// }
	// return payMsg;
	// }
	//
	...
	// @ResponseBody
	// public Object refund(HttpServletRequest request, HttpServletResponse
	// response) {
	// String pay_channel = request.getParameter("pay_channel");
	// String merchant_num = request.getParameter("merchant_num");
	// String trace_num = request.getParameter("trace_num");
	...
	// Strings.isNullOrEmpty(merchant_num)
	// || Strings.isNullOrEmpty(trace_num) ||
	// Strings.isNullOrEmpty(refund_amount)
	// || Strings.isNullOrEmpty(terminal_unique_no)) {
	// return new Message(ResultCode.FAIL.name(),
	// ErrorCode.PARAM_IS_NULL.name(), "传递参数为空！", null);
	// }
	// Message payMsg = null;
	// try {
	// AlipayRefundReqData alipayRefundReqData = new
	// AlipayRefundReqData(AlipayConfig.partner, trace_num,
	// refund_amount);
