 ...
 */
@Controller
public class WchatpaySLController extends BaseController{
	Logger log = LoggerFactory.getLogger(WchatpaySLController.class);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/pay/wechatpaysl/create")
	@ResponseBody
	public Object barCreate(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//主商户配置
			SysWechatConfigWithBLOBs sysWechatConfigPar = loadParentWchatpayConf(sysWechatConfigSub);
			if (sysWechatConfigPar== null) {
				return new Message(ResultCode.FAIL.name(), "parent_payconfig_not_find", "无服务商支付信息配置", null);
			}
...
//			IdWorker iw = new IdWorker(idepo);
			...
			ScanPayReqData scanPayReqData = new ScanPayReqData(dynamic_id,body, "attach data",
					orderNo, totalFee, terminal_unique_no, ip, "bar pay",sysWechatConfigPar, sysWechatConfigSub);
			...
			payMsg = wechatPayService.barPay(scanPayReqData, sysWechatConfigPar,dtoMap);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/pay/wechatpaysl/scan/create")
	@ResponseBody
...
//			IdWorker iw = new IdWorker(idepo);
			...
			ScanCodePayReqData scanCodePayReqData = new ScanCodePayReqData("wechat pay", orderNo, totalFee,
					terminal_unique_no, ip, "wechat pay", "wechat pay test", sysWechatConfigPar, sysWechatConfigSub);
			payMsg = wechatPayService.unifiedOrder(scanCodePayReqData, sysWechatConfigPar,user_order_no);
		} catch (Exception e) {
			log.error("微信支付出现异常：", e);
			return new Message(ResultCode.FAIL.name(), ErrorCode.SYSTEM_EXCEPTION.name(), "支付出现异常！", null);
		}
	 ...
	 * @param response
	 */
	@RequestMapping(value = "/pay/wechatpaysl/wap/create")
	public Object wapCreate(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if(payMsg.getResult_code().equals("SUCCESS")){
				...
				String paySign=  Signature.getSign(wxPayParamMap, sysWechatConfigPar.getApiSecret());
	 ...
	 * @return
	 */
	@RequestMapping(value = "/pay/wechatpaysl/scan/close")
	@ResponseBody
	public Object closeOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 支付请求
			CloseOrderReqData closeOrderReqData = new CloseOrderReqData(merchant_num, out_trade_no, sysWechatConfigPar,sysWechatConfigSub);
			payMsg = wechatPayService.closeOrder(closeOrderReqData, sysWechatConfigPar,user_order_no);
		} catch (Exception e) {
			log.error("微信支付出现异常：", e);
			return new Message(ResultCode.FAIL.name(), ErrorCode.SYSTEM_EXCEPTION.name(), "支付出现异常！", null);
		}
	 ...
	 */
	@RequestMapping(value = "/pay/wechatpaysl/reverse")
	@ResponseBody
	public Object reverse(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 支付请求
			ReverseReqData reverseReqData = new ReverseReqData(merchant_num, out_trade_no, sysWechatConfigPar,sysWechatConfigSub);
			payMsg = wechatPayService.reverse(reverseReqData, sysWechatConfigPar,user_order_no);
		} catch (Exception e) {
			log.error("微信支付出现异常：", e);
			return new Message(ResultCode.FAIL.name(), ErrorCode.SYSTEM_EXCEPTION.name(), "支付出现异常！", null);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/pay/wechatpaysl/query")
	@ResponseBody
	public Object query(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ScanPayQueryReqData scanPayQueryReqData = new ScanPayQueryReqData("", out_trade_no,
					sysWechatConfigPar,sysWechatConfigSub);
			payMsg = wechatPayService.query(scanPayQueryReqData,sysWechatConfigPar,dtoMap);
		} catch (Exception e) {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/pay/wechatpaysl/refund")
	@ResponseBody
...
//			IdWorker iw = new IdWorker(idepo);
			...
			RefundReqData refundReqData = new RefundReqData(transactionID, out_trade_no, terminal_unique_no, refundNo,
					...
					Integer.valueOf(AmountUtils.changeY2F(refund_amount)), merchant_num, "CNY", sysWechatConfigPar,sysWechatConfigSub);

			payMsg = wechatPayService.refund(refundReqData, sysWechatConfigPar,user_order_no);
		} catch (Exception e) {
			log.error("微信退款出现异常：", e);
			return new Message(ResultCode.FAIL.name(), ErrorCode.SYSTEM_EXCEPTION.name(), "支付出现异常！", null);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/pay/wechatpaysl/refundQuery")
	@ResponseBody
	public Object refundQuery(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			RefundQueryReqData refundQueryReqData = new RefundQueryReqData(out_trade_no, terminal_unique_no,sysWechatConfigPar,sysWechatConfigSub);
			reqParamMap.put("merchant_name", sysMerchant.getMerchantName());
			payMsg = wechatPayService.refundQuery(refundQueryReqData, sysWechatConfigPar,reqParamMap);
	 ...
	 * @param response
	 */
	@RequestMapping("/pay/wechatpaysl/scan/notify")
	...
	@RequestMapping("/pay/wechatpaysl/wap/notify")
	...
	//查询主账号配置信息
	private SysWechatConfigWithBLOBs loadParentWchatpayConf(SysWechatConfigWithBLOBs sysWechatConfigSub){
		if( SysWechatConfig.mchType_mch == sysWechatConfigSub.getMchType() && sysWechatConfigSub.getParentMchId() != null){
			return  sysWechatConfigService.findByMerchantNo(sysWechatConfigSub.getParentMchId());
		}else{
			return null;
		}
	}
