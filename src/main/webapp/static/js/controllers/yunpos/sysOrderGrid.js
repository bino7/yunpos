app.controller('OrderListCtrl',  function($scope, $http, $state, $stateParams) {
    ...
    $scope.gridOptions = {
          ...
          //  enablePinning: true,
            columnDefs: [
               ...
               {field: 'sourceType', displayName: '来源类型', enableCellEdit: false, width: 120 }, 
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText,payStatus,merchantType,sourceType) {
        setTimeout(function () {
            var data;
            if (searchText ||payStatus || merchantType || sourceType) {
              var ft = searchText.toLowerCase();
              var select_payStatus = document.getElementById("select_payStatus").value;
      		  var merchant_type = document.getElementById("merchant_type").value;
                ...
                data = $scope.orderData.filter(function(item) {
                	if(JSON.stringify(item.payStatusStr).indexOf(select_payStatus) !=-1 && JSON.stringify(item.merchantType).indexOf(merchant_type) != -1 && JSON.stringify(item).toLowerCase().indexOf(filter_text) != -1 && JSON.stringify(item.sourceType).toLowerCase().indexOf(source_type) != -1){
              		  return item ;}
                });
...
//        	$scope.setPagingData($scope.orderData, $scope.pagingOptions.currentPage, $scope.pagingOptions.pageSize);
            $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage, $scope.filterText,$scope.option_payStatus,$scope.merchantType,$scope.source_type);
        }
    }, true);
	 ...
	 $scope.search = function() {
          ...
          var data = $scope.orderData.filter(function(item) {
        	/* alert(item.payStatus);*/
        	  ...
        	  if(JSON.stringify(item.payStatusStr).indexOf(select_payStatus) !=-1 && JSON.stringify(item.merchantType).indexOf(merchant_type) != -1 
        			  && JSON.stringify(item).toLowerCase().indexOf(filter_text) != -1 && JSON.stringify(item.sourceType).toLowerCase().indexOf(source_type) != -1){
        		  return item ;}
         });
